...
namespace UMS.Reflection
{
    ...
    {
        private static Dictionary<Config, Dictionary<Type, MetaType>> _configMetaTypes =
            new Dictionary<Config, Dictionary<Type, MetaType>>();

        public static MetaType Get(Config config, Type type)
        {
            Dictionary<Type, MetaType> metaTypes;
            lock (typeof(MetaType))
            {
                if (_configMetaTypes.TryGetValue(config, out metaTypes) == false)
                    metaTypes = _configMetaTypes[config] = new Dictionary<Type, MetaType>();
            }

            MetaType metaType;
            if (metaTypes.TryGetValue(type, out metaType) == false)
            {
                metaType = new MetaType(config, type);
        ...
        public static void ClearCache()
        {
            lock (typeof(MetaType))
            {
                _configMetaTypes = new Dictionary<Config, Dictionary<Type, MetaType>>();
            }
        }

        private MetaType(Config config, Type reflectedType)
        {
        ...
        private static void CollectProperties(Config config, List<MetaProperty> properties, Type reflectedType)
        {
            // do we require a [SerializeField] or [Property] attribute?
            bool requireOptIn = config.DefaultMemberSerialization == MemberSerialization.OptIn;
            bool requireOptOut = config.DefaultMemberSerialization == MemberSerialization.OptOut;
            ...
            foreach (var member in members)
            {
                ...
                if (config.IgnoreSerializeAttributes.Any(t => PortableReflection.HasAttribute(member, t)))
                {
                    continue;
                }
                ...
                if (requireOptIn &&
                    !config.SerializeAttributes.Any(t => PortableReflection.HasAttribute(member, t)))
                {
                    continue;
                }
                ...
                if (requireOptOut &&
                    config.IgnoreSerializeAttributes.Any(t => PortableReflection.HasAttribute(member, t)))
                {
                    continue;
                }
                ...
                if (property != null)
                {
                    ...
                    {
                        properties.Add(new MetaProperty(config, property));
                    }
                }
                else if (field != null)
                {
                    ...
                    {
                        properties.Add(new MetaProperty(config, field));
                    }
                }
            }
        ...
        private static bool IsAutoProperty(PropertyInfo property, MemberInfo[] members)
        {
            return
                ...
                PortableReflection.HasAttribute(
        ...
        /// </param>
        private static bool CanSerializeProperty(Config config, PropertyInfo property, MemberInfo[] members, bool annotationFreeValue)
        {
            ...
            if (config.IgnoreSerializeTypeAttributes.Any(t => PortableReflection.HasAttribute(property.PropertyType, t)))
            {
                return false;
            }
            ...
            if (config.SerializeAttributes.Any(t => PortableReflection.HasAttribute(property, t)))
            {
                return true;
            }
        ...
        private static bool CanSerializeField(Config config, FieldInfo field, bool annotationFreeValue)
        {
            // We don't serialize delegates
            ...
            // Don't serialize members of types that themselves aren't to be serialized.
            if (config.IgnoreSerializeTypeAttributes.Any(t => PortableReflection.HasAttribute(field.FieldType, t)))
            {
                return false;
            }
            ...
            if (config.SerializeAttributes.Any(t => PortableReflection.HasAttribute(field, t)))
            {
                return true;
            }
        ...
        {
            AOT.AOTCompilationManager.AotCandidateTypes.Add(ReflectedType);
            if (!throwException)
                return;
